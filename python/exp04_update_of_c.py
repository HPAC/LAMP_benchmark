...
def exp04_update_of_c(b, n):
    ...
    logger.info('Syrk correctness: {}'.format(
        tf.debugging.assert_near(tf.linalg.band_part(res5, -1, 0), tf.linalg.band_part(res6, -1, 0)), rtol=1e-05,
        atol=1e-08))
    ...
    logger.info('Syr2k correctness: {}'.format(
        tf.debugging.assert_near(tf.linalg.band_part(res7, -1, 0), tf.linalg.band_part(res8, -1, 0)), rtol=1e-05,
        atol=1e-08))
