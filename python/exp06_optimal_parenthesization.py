...
def exp06_optimal_parenthesization(b, n):
    ...
    logger.info('mc_l_r correctness: {}'.format(tf.experimental.numpy.allclose(res1, res2)))

    C = tf.random.normal([n, n_5])
    ...
    logger.info('mc_r_l correctness: {}'.format(tf.experimental.numpy.allclose(res3, res4)))

    A = tf.random.normal([n, n])
    B = tf.random.normal([n, n_5])
    C = tf.random.normal([n_5, n])
    D = tf.random.normal([n, n])
    ...
    logger.info('mc_mixed correctness: {}'.format(tf.experimental.numpy.allclose(res5, res6)))
